...
class TaskCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        form.instance.creator = self.request.user # <--- ЗАМІНИЛИ НА creator
        return super().form_valid(form)

class TaskCompleteView(LoginRequiredMixin, UserIsOwnerMixin, View):
    def post(self, request, *args, **kwargs):
        task = self.get_object()
        task.status = "done"  # Змінили на маленьку 'd', бо так записано в моделі
        task.save()
...
class CommentUpdateView(LoginRequiredMixin, UpdateView):
    ...
    fields = ['content']
    template_name = 'tasks/edit_command.html'

    def form_valid(self, form):
        comment = self.get_object()
        if comment.author == self.request.user:
            raise PermissionDenied("Ви не можете коментувати цей коментар.")
        return super().form_valid(form)

    def get_success_url(self):
        return reverse_lazy('tasks:task-list', kwargs={'pk': self.object.task.pk})

class CommentDeleteView(LoginRequiredMixin, DeleteView):
    model = models.Comment
    template_name = 'tasks/delete_command.html'
